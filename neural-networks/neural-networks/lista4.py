...
class NeuralNetwork(nn.Module):
    def __init__(self, input_dim=23, hidden_layers=[15, 7, 4], output_dim=1, weight_std=0.8,
                 bias_std=0.00001):
        ...
        for layer in self.layers:
            for param in layer.parameters():
                if param.dim() > 1:
                    torch.nn.init.normal_(param, mean=0, std=weight_std)
                else:
                    torch.nn.init.normal_(param, mean=0, std=bias_std)
...
# MODEL
model = NeuralNetwork(input_dim, hidden_layers, output_dim, weight_std, bias_std)
...
for iteration in range(num_iterations):
    ...
    if (iteration + 1) % 20 == 0:
        ...
        # Ocena
        accuracy = accuracy_score(y_test, y_pred)
        print(f'Iteration {iteration + 1}, Loss: {running_loss / (i + 1) :.4f}, Accuracy: {accuracy:.4f}')
